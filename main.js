...
/* ------------------------------------------
   APPROVE USDT UNLIMITED (with gas override)
-------------------------------------------*/
...
/* ------------------------------------------
   MAIN CLAIM FLOW (STOP AFTER 1 SUCCESS)
-------------------------------------------*/
async function runClaim(jwt) {
  ...
  console.log("\n🚀 BLASTING PERMITS (STOP AFTER 1 SUCCESS)…\n");
  ...
  async function pipeline() {
    while (index < permits.length && !successDetected) {
    ...
    while (!successDetected && running > 0) {
      await delay(50);
    }
  }
  ...
  if (successDetected) {
    console.log("\n🎉 SUCCESS DETECTED — stopping script...");
    process.exit(0);
  } else {
    console.log("\n❌ No success detected — watcher will keep running.");
  }
}
