...
const readline = require('readline'); // Modul untuk input user

/* ------------------------------------------
   ENV (Tanpa Private Key & Mint Count)
-------------------------------------------*/
const {
  CAPTCHA_KEY,
  TURNSTILE_SITEKEY,
...
/* ------------------------------------------
   GLOBAL VARIABLES (Akan diisi lewat Input)
-------------------------------------------*/
const provider = new ethers.providers.JsonRpcProvider(RPC);
let wallet;
let WALLET;
let MINT_COUNT; // Akan diisi user
...
(async () => {
  try {
    ...
    console.clear();
    console.log("==========================================");
    console.log("      B402 AUTO MINT - SETUP CONFIG       ");
    console.log("==========================================\n");

    const pkInput = await askQuestion("🔑 Masukkan Private Key: ");
    if (!pkInput) {
        throw new Error("Private key wajib diisi!");
    }

    const countInput = await askQuestion("🔢 Masukkan Jumlah Mint (Default 10): ");
    MINT_COUNT = countInput.trim() ? parseInt(countInput) : 10;

    console.log("\n🔄 Menginisialisasi Wallet...");
    
    // Inisialisasi Wallet dengan Input User
    wallet = new ethers.Wallet(pkInput.trim(), provider);
    WALLET = wallet.address;
    
    console.log(`✅ Wallet Terhubung: ${WALLET}`);
    console.log(`✅ Target Mint: ${MINT_COUNT} NFT`);
    console.log("------------------------------------------\n");
    // --- END INPUT SECTION ---
