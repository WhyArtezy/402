...
/* ------------------------------------------
   TERMINAL INPUT (PRIVATE KEY + MINT COUNT)
-------------------------------------------*/
...
async function getUserInput() {
  console.log("=========== B402 AUTO MINT ===========");

  const pk = await ask("Masukkan Private Key: ");
  const mint = await ask("Berapa kali mint? (misal 10): ");

  rl.close();

  return {
    PRIVATE_KEY_INPUT: pk.trim(),
    MINT_COUNT_INPUT: Number(mint.trim())
  };
}
...
(async () => {
  ...
  const PRIVATE_KEY = input.PRIVATE_KEY_INPUT;
  const MINT_COUNT = input.MINT_COUNT_INPUT;

  console.log("--------------------------------------");
  console.log("Wallet:", new ethers.Wallet(PRIVATE_KEY).address);
  console.log("Mint:", MINT_COUNT + "x");
  console.log("Status: Watching network...");
  console.log("--------------------------------------\n");
    ...
    function mintPermit(p, i) {
      return axios.post(
        ...
        .catch(err => {
          ...
          const low = JSON.stringify(msg).toLowerCase();
